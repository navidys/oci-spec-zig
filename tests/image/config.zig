...
test "image config" {
    ...
    try testing.expectEqualStrings(config1.created, "2015-10-31T22:22:56.015925234Z");
    try testing.expectEqualStrings(config1.author.?, "Alyssa P. Hacker <alyspdev@example.com>");
    try testing.expectEqual(config1.architecture, image.Arch.Amd64);
    try testing.expectEqual(config1.os, image.OS.Linux);
    try testing.expectEqualStrings(config1.config.?.User.?, "alice");
